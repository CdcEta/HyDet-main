...
class HRSCDataset(BaseDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if self.classwise:
            self.catid2label = {}
            for i, cls_name in enumerate(self._metainfo['classes']):
                if isinstance(cls_name, str) and cls_name.startswith('class_'):
                    raw_id = cls_name[len('class_'):]
                    self.catid2label[raw_id] = i
                    self.catid2label[raw_id.lstrip('0')] = i
        else:
    ...
    def parse_data_info(self, img_info: dict) -> Union[dict, List[dict]]:
        ...
        for obj in raw_ann_info.findall('HRSC_Objects/HRSC_Object'):
            ...
            if self.classwise:
                ...
                if class_id not in self.catid2label:
                    class_id = class_id.lstrip('0')
                if class_id not in self.catid2label:
                    continue
            ...
            try:
                head = [
                    int(float(obj.find('header_x').text)),
                    int(float(obj.find('header_y').text))
                ]
            except (TypeError, ValueError):
                head = [0, 0]
