...
def _collect_variant(root: Path, variant: str) -> Dict[str, object]:
    ...
    for split in ['all', 'base', 'novel']:
        metrics = _find_log_metrics(root / f'{variant}_eval_{split}')
        if 'mAP' in metrics:
            row[f'{split}_mAP'] = metrics['mAP']
        if 'AP50' in metrics:
            row[f'{split}_AP50'] = metrics['AP50']
    return row
...
def main() -> None:
    ...
    headers = ['Method', 'base_mAP', 'novel_mAP', 'all_mAP', 'base_AP50', 'novel_AP50', 'all_AP50']
